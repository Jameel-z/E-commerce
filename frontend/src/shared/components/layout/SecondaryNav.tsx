...
import { Search, ChevronDown, Phone, LayoutGrid, ShoppingBag, Menu, X } from "lucide-react";
...
export function SecondaryNav() {
  ...
  const [expandedId, setExpandedId] = useState<number | null>(null); // mobile tap-expand
  ...
  // Autocomplete — fires after 3 characters with 300 ms debounce
  useEffect(() => {
  ...
  const close = () => { setIsCatOpen(false); setHoveredId(null); setExpandedId(null); };

  // ── Shared autocomplete results list ─────────────────────────────────────────
      ...
      {suggestions.map((product) => (
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-foreground line-clamp-1 leading-tight">{product.name}</p>
  ...
  // ── Category rows (reused in both desktop dropdown and mobile panel) ──────────
  const CategoryRows = ({ onNavigate }: { onNavigate: () => void }) => (
    <>
      <Link
        href="/products"
      ...
      {tree.map((cat) => (
        <div key={cat.id}>
          {/* Desktop: hover; Mobile: tap chevron to expand */}
          <div
          ...
          >
            <Link
              ...
              className="flex-1 flex items-center px-4 py-2.5 text-sm font-medium hover:bg-muted transition-colors gap-2"
              onClick={onNavigate}
            >
              <span className="flex-1">{cat.name}</span>
              {cat.children.length > 0 && (
                <span className={`text-[11px] font-bold px-1.5 py-0.5 rounded-md transition-colors duration-150 hidden sm:inline ${
                  hoveredId === cat.id ? "bg-primary text-primary-foreground" : "bg-primary/15 text-primary"
                }`}>
                  {cat.children.length} ›
                </span>
              )}
          ...
          {/* Desktop hover children */}
          {hoveredId === cat.id && cat.children.length > 0 && (
            <div className="hidden sm:block bg-muted/50 border-t border-b">
          ...
          {/* Mobile tap-expanded children */}
          {expandedId === cat.id && cat.children.length > 0 && (
            <div className="sm:hidden bg-muted/30 border-t border-b">
              {cat.children.map((child) => (
                <Link
                  key={child.id}
                  href={buildCategoryUrl([child.name])}
                  className="flex items-center pl-6 pr-4 py-2.5 text-sm text-muted-foreground hover:text-foreground hover:bg-muted transition-colors gap-2"
                  onClick={onNavigate}
                >
                  <span className="w-1 h-1 rounded-full bg-primary/60 flex-shrink-0" />
                  {child.name}
  ...
  return (
    <div className="border-b bg-muted/50 relative" ref={navRef}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center h-9 gap-3">
          ...
          <div className="relative flex-shrink-0" ref={desktopDropdownRef}>
            ...
            >
              {/* Mobile: 3-line hamburger / close */}
              {isCatOpen
                ? <X className="h-4 w-4 sm:hidden" />
                : <Menu className="h-4 w-4 sm:hidden" />
              }
              {/* Desktop: grid icon + label */}
          ...
          <form onSubmit={handleSearch} className="hidden sm:flex items-center gap-2 flex-1">
            <div className="relative flex-1" ref={searchRef}>
              ...
              {showSuggestions && (
                <div className="absolute top-full left-0 right-0 mt-1 bg-background border rounded-lg shadow-xl z-50 overflow-hidden">
                  ...
                  {suggestions.length > 0 ? <SuggestionsList /> : !loadingSuggestions ? (
                    <div className="px-3 py-4 text-sm text-muted-foreground text-center">No products found</div>
                  ) : null}
                </div>
              )}
            </div>
            <button type="submit" className="h-8 px-3 text-xs font-semibold bg-primary text-primary-foreground rounded-md hover:bg-primary/90 transition-colors flex-shrink-0">
              Search
            </button>
          </form>
      ...
      {/* ── Mobile full-width panel (search + categories) ─────────────────── */}
      {isCatOpen && (
        <div className="sm:hidden absolute left-0 right-0 top-full bg-card border-b shadow-2xl z-50 max-h-[70vh] overflow-y-auto">

          {/* Search row */}
          <div className="sticky top-0 bg-card px-4 py-3 border-b z-10">
            <form onSubmit={handleSearch} className="flex gap-2">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  onKeyDown={(e) => e.key === "Escape" && setShowSuggestions(false)}
                  placeholder="Search products…"
                  className="w-full h-9 pl-9 pr-3 text-sm bg-background border rounded-md focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-colors"
                  autoComplete="off"
                  autoFocus
                />
              </div>
              <button type="submit" className="h-9 px-4 text-sm font-semibold bg-primary text-primary-foreground rounded-md hover:bg-primary/90 transition-colors flex-shrink-0">
                Search
              </button>
            </form>
            ...
            {/* Inline suggestions */}
            {showSuggestions && (
              <div className="mt-2 bg-background border rounded-lg overflow-hidden shadow-lg">
                <div className="px-3 py-2 bg-muted/60 border-b">
                  <span className="text-xs font-bold uppercase tracking-wide text-muted-foreground">
                    {loadingSuggestions ? "Searching…" : "Products"}
                  </span>
